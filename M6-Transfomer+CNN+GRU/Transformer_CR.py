...
class TransformerCNNGRU(BaseModel):  #basemodel 是父类
    def __init__(self, config, vocab_size): #__init__(self, config, vocab_size, word_vectors):
        super(TransformerCNNGRU, self).__init__(config=config, vocab_size=vocab_size, word_vectors=None)
    ...
    def build_model(self):
        ...
        with tf.name_scope("embedding"):
            ...
            if self.word_vectors is not None:
                ...
                # print("embedding_w",embedding_w) #<tf.Variable 'embedding_w:0' shape=(8619, 200) dtype=float32_ref>
    #         # 利用词嵌入矩阵将输入的数据中的词转换成词向量，维度[batch_size, sequence_length, embedding_size]
            embedded_words = tf.nn.embedding_lookup(embedding_w, self.inputs)
            # print("embedded_words",embedded_words) #shape=(?, ?, 200),
            ...
            for i in range(self.config["num_blocks"]):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    ...
                    with tf.name_scope("feed_forward"):
                        # 维度[batch_size, sequence_length, embedding_size] shape=(64, 700, 200)
                        embedded_representation = self._feed_forward(multihead_atten,
                                                                     [self.config["filters"],
                                                                      self.config["embedding_size"]])
    #
                    with tf.name_scope("Bi-GRU"):
                        # print("embedded_representation",embedded_representation)
                        word_cell_fw = tf.contrib.rnn.GRUCell(self.config["word_hidden_dim"])
                        word_Cell_fw = tf.contrib.rnn.DropoutWrapper(word_cell_fw, self.config["keep_prob"])
                        word_cell_bw = tf.contrib.rnn.GRUCell(self.config["word_hidden_dim"])
                        word_Cell_bw = tf.contrib.rnn.DropoutWrapper(word_cell_bw, self.config["keep_prob"])

                        wordoutput, _ = tf.nn.bidirectional_dynamic_rnn(cell_fw=word_Cell_fw, cell_bw=word_Cell_bw,
                        ...
                        print("wordoutput",wordoutput)

                        embedded_representation=embedded_representation+wordoutput
                        print("embedded_representation",embedded_representation)
    ...
    def _multihead_attention(self, inputs, queries, keys, num_units=None):
        ...
        # tf.layers.dense可以做多维tensor数据的非线性映射，在计算self-Attention时，一定要对这三个值进行非线性映射，tf.layers.dense为全连接层  相当于添加一个层
    ...
    def _feed_forward(self, inputs, filters):
        ...
        # 外层
        params = {"inputs": outputs, "filters": filters[1], "kernel_size": 1, #过滤器（卷积核）的数目，
                  "activation": None, "use_bias": True}
        #filters：过滤器（卷积核）的数目 kernel_size：卷积核的大小，卷积核本身应该是二维的，这里只需要指定一维，因为第二个维度即长度与词向量的长度一致，
